...
**Critical**: 
- `dist/output.css` must be committed (Tailwind build output)
...
8. **Forgetting to commit `dist/output.css`**: GitHub Pages deployment needs this file committed to work properly.

9. **Using old event patterns**: Don't add manual `addEventListener` for standard UI actions - use ActionBus `data-action` instead.
