...
## Dev Minimum (Blocking)

Aktueller PR-Blocking-Fokus ist ein kleines, stabiles Smoke-Set statt der kompletten E2E-Suite.

- Command: `npm run test:e2e:smoke`
- Alias: `npm run test:e2e:quick`
- Scope: `tests/e2e/smoke/dev-minimum.spec.js`
- Letzter Lauf: 5/5 gruen (`npm run test:e2e:smoke`, lokal Chromium)

Die volle Suite (`npm run test:e2e`) bleibt fuer Nightly/Release und gezielte Regression-Phasen erhalten.

CI-Policy (ci.yml):

- PR/Push: Blocking ist nur E2E Smoke (Chromium).
- Full Regression laeuft auf main, nightly (schedule), manuell via workflow_dispatch (fullRegression=true) oder explizit per [test-all].
...
- [x] tests/e2e/apps/finder-vdom-tabs.spec.js - auf Multi-Window-Finder und aktuelle VDOM-Tab-Container umgestellt (gezielter Re-Run gruen)
- [x] tests/e2e/finder/finder-new-features.spec.js - Sort-Menu/View-Trigger/Navigations-Selektoren aktualisiert; bis auf Persistenz-Fall gruen
- [x] tests/e2e/text-editor/text-editor-tabs.spec.js - wieder auf tatsaechlich gerenderten Text-Modal-Tab-Container ausgerichtet (gezielter Re-Run gruen)

## Fehlgeschlagene Tests (51)

Status-Hinweis: gezielter Re-Run `tests/e2e/finder/finder-tabs-ghost-fix.spec.js` aktuell 2/8 gruen (6 offen).
...
- [ ] tests/e2e/finder/finder-new-features.spec.js:241:9 › FinderView New Features › State Persistence: Remembers view mode and sort settings - Error: expect(locator).toHaveAttribute(expected) failed (aktueller Persistenz-Fall)
