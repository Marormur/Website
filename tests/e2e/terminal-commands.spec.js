...
test.describe('Terminal - Command and Path Handling', () => {
    test.beforeEach(async ({ page, baseURL }) => {
        ...
        await expect(page.locator('#terminal-modal')).toBeVisible({ timeout: 5000 });
        ...
        await page.waitForTimeout(500); // Give terminal time to initialize
    });
    ...
    async function executeCommand(page, command) {
        ...
        await page.waitForTimeout(200); // Wait for command execution
    }
    ...
    test('should display welcome message on load', async ({ page }) => {
        ...
        const hasWelcome = output.some(line =>
            line.includes('Willkommen') || line.includes('help')
        );
    ...
    test('should execute help command', async ({ page }) => {
        ...
        const output = await getOutputLines(page);
        const hasHelpText = output.some(line =>
            line.includes('Verfügbare Befehle') || line.includes('help')
        );
    ...
    test('should execute cat with simple filename', async ({ page }) => {
        ...
        const output = await getOutputLines(page);
        const hasContent = output.some(line =>
            line.includes('Willkommen auf Marvins Portfolio')
        );
    ...
    test('should execute cat with ./ prefix', async ({ page }) => {
        ...
        const hasError = output.some(line =>
    ...
    test('should cat file in subdirectory', async ({ page }) => {
        ...
        const output = await getOutputLines(page);
        const hasContent = output.some(line =>
            line.includes('Willkommen im Terminal')
        );
        expect(hasContent).toBe(true);
    });
