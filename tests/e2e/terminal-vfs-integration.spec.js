...
test.describe('Terminal VirtualFS Integration', () => {
    ...
    test('Terminal sessions share same VirtualFS instance', async ({ page }) => {
        ...
        const result = await page.evaluate(() => {
            const win = window.WindowRegistry?.getAllWindows('terminal')?.[0];
            const session = win?.activeSession;
            if (!session || !window.VirtualFS) return null;

            const files = window.VirtualFS.listDirectory(session.vfsCwd);
            return files.some(f => f.name === 'testfile.txt');
        });
    ...
    test('Finder and Terminal share same VirtualFS', async ({ page }) => {
        ...
        const finderDockItem = page.locator('.dock-item[data-window-id="finder-modal"]');
        await finderDockItem.click();

        await page.waitForFunction(
            () => {
                return window.WindowRegistry?.getAllWindows('finder')?.length === 1;
            },
            { timeout: 5000 }
        );
        ...
        const hasFile = await page.evaluate(() => {
            ...
            const files = window.VirtualFS.listDirectory(session.vfsCwd);
            return files.some(f => f.name === 'shared-file.txt');
        });
    ...
    test('vfsCwd persists across session restore', async ({ page }) => {
        ...
        // Check restored session has correct cwd
        const restoredCwd = await page.evaluate(() => {
            const win = window.WindowRegistry?.getAllWindows('terminal')?.[0];
            return win?.activeSession?.vfsCwd || null;
        });

        expect(restoredCwd).toBe('/home/marvin/Documents');
    });
