...
test('visual check: homepage in Firefox vs Chrome', async ({ page, browserName }) => {
    ...
    const menubarDisplay = await page.locator('.menubar').evaluate(el => window.getComputedStyle(el).display);
    const menubarBg = await page.locator('.menubar').evaluate(el => window.getComputedStyle(el).backgroundColor);
    
    console.log(`[${browserName}] menubar display: ${menubarDisplay}, background: ${menubarBg}`);
    ...
    if (dockTray > 0) {
        const dockBg = await page.locator('#dock .dock-tray').first().evaluate(el => window.getComputedStyle(el).backgroundColor);
