...
(function () {
    ...
    function createModuleProxy<T extends string[]>(moduleName: string, methods: T): ModuleProxy<T> {
        const proxy = {} as ModuleProxy<T>;

        methods.forEach(method => {
            (proxy as Record<string, ModuleMethod>)[method] = function (
                ...args: unknown[]
            ): unknown {
    ...
    const API = {
        ...
        i18n: {
            translate: (key: string, fallback?: string): string => {
                ...
                if (
                    appI18n &&
                    typeof (appI18n as Record<string, unknown>)['translate'] === 'function'
                ) {
                    const result = (
                        (appI18n as Record<string, unknown>)['translate'] as (k: string) => string
                    )(key);
                    return result === key && fallback ? fallback : result;
                }
                return fallback || key;
            },
            setLanguagePreference: (lang: unknown) =>
                callWindowMethod('appI18n', 'setLanguagePreference', lang),
            getLanguagePreference: (): string =>
                (callWindowMethod('appI18n', 'getLanguagePreference') as string) || 'system',
            getActiveLanguage: (): string =>
                (callWindowMethod('appI18n', 'getActiveLanguage') as string) || 'en',
