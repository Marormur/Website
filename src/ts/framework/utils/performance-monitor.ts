...
export class ComponentPerformanceMonitor {
    ...
    enable(): void {
        this.enabled = true;
        logger.debug('FRAMEWORK', '[MacUI Performance] Monitoring enabled');
    }
    ...
    disable(): void {
        this.enabled = false;
        logger.debug('FRAMEWORK', '[MacUI Performance] Monitoring disabled');
    }
    ...
    markRenderEnd(componentName: string): void {
        ...
        try {
            ...
            metrics.renderTime =
                (metrics.renderTime * (metrics.renderCount - 1) + measure.duration) /
                metrics.renderCount;
            ...
            if (measure.duration > 16) {
                logger.warn(
                    'FRAMEWORK',
                    `[MacUI Performance] Slow render: ${componentName} took ${measure.duration.toFixed(2)}ms`
                );
            }
        } catch {
    ...
    getReport(): {
        ...
        const averageRenderTime =
            totalRenders > 0
                ? components.reduce((sum, m) => sum + m.renderTime * m.renderCount, 0) /
                  totalRenders
                : 0;
    ...
    clear(): void {
        ...
        logger.debug('FRAMEWORK', '[MacUI Performance] Metrics cleared');
    }
    ...
    logReport(): void {
        ...
        logger.group('FRAMEWORK', '[MacUI Performance Report]');
        logger.debug('FRAMEWORK', `Total renders: ${report.totalRenders}`);
        logger.debug('FRAMEWORK', `Average render time: ${report.averageRenderTime.toFixed(2)}ms`);
        logger.debug('FRAMEWORK', `Memory trend: ${report.memoryTrend}`);

        if (report.slowComponents.length > 0) {
            logger.group('FRAMEWORK', 'Slow components (>16ms):');
            report.slowComponents.forEach(m => {
                logger.debug(
                    'FRAMEWORK',
                    `${m.componentName}: ${m.renderTime.toFixed(2)}ms avg (${m.renderCount} renders)`
                );
            });
            logger.groupEnd();
        }
