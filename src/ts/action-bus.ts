...
(function () {
  ...
  const type = params['itemType'] || params['type'];
    ...
    'session:export': () => {
      const W = window as unknown as Window & { SessionManager?: { exportSession?: () => string | null }; appI18n?: { translate?: (key: string) => string } };
    ...
    'session:import': () => {
      const W = window as unknown as Window & { SessionManager?: { importSession?: (json: string) => boolean }; appI18n?: { translate?: (key: string) => string } };
      const translate = W.appI18n?.translate || ((k: string) => k);
