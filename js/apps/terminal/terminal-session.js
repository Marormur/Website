 ...
 */
class TerminalSession extends base_tab_js_1.BaseTab {
    constructor(config) {
        super({
        ...
        try {
            const home = virtual_fs_js_1.VirtualFS.getFolder('/home/marvin');
            if (!home || !home.children) {
                console.warn('[TerminalSession] VirtualFS missing defaults, resetting');
                virtual_fs_js_1.VirtualFS.reset();
            }
        } catch (error) {
    ...
    completePathArgument(cmd, rawArg, opts) {
        ...
        const folder = virtual_fs_js_1.VirtualFS.getFolder(dirForResolve);
        try {
    ...
    listDirectory(path) {
        ...
        const item = virtual_fs_js_1.VirtualFS.get(target);
        if (!item) {
        ...
        const entries = virtual_fs_js_1.VirtualFS.list(target);
    ...
    changeDirectory(path) {
        ...
        const folder = virtual_fs_js_1.VirtualFS.getFolder(target);
        if (!folder) {
    ...
    catFile(filename) {
        ...
        const file = virtual_fs_js_1.VirtualFS.getFile(target);
    ...
    touch(path) {
        ...
        const target = this.vfsResolve(path);
        const existing = virtual_fs_js_1.VirtualFS.get(target);
        if (existing) {
        ...
        const parent = virtual_fs_js_1.VirtualFS.getFolder(parentPath);
        if (!parent) {
            this.addOutput(`Übergeordnetes Verzeichnis nicht gefunden: ${parentPath}`, 'error');
            return;
        }
        const ok = virtual_fs_js_1.VirtualFS.createFile([...parts, name].join('/'), '');
    ...
    mkdir(path) {
        ...
        const ok = virtual_fs_js_1.VirtualFS.createFolder([...parts, name].join('/'));
    ...
    rm(path) {
        ...
        const target = this.vfsResolve(path);
        if (!virtual_fs_js_1.VirtualFS.get(target)) {
            this.addOutput(`Nicht gefunden: ${target}`, 'error');
            return;
        }
        const ok = virtual_fs_js_1.VirtualFS.delete(target);
