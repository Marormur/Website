...
exports.FinderView = void 0;
const base_tab_js_1 = require('./base-tab.js');
const virtual_fs_js_1 = require('./virtual-fs.js');
const preview_instance_manager_1 = __importDefault(require('./preview-instance-manager'));
const ROOT_FOLDER_NAME = 'Computer';
class FinderView extends base_tab_js_1.BaseTab {
    constructor(config) {
        super({
            type: 'finder-view',
    ...
    _attachEvents() {
        ...
        document.addEventListener('click', e => {
            if (!sortToggleBtn?.contains(e.target) && !sortMenu?.contains(e.target)) {
                sortMenu?.classList.add('hidden');
            }
        });
        // Favorite toggle
    ...
    renderBreadcrumbs() {
        ...
        this.dom.breadcrumbs.querySelectorAll('button[data-action="goto"]').forEach(btn => {
            btn.addEventListener('click', () => {
        ...
        const rootBtn = this.dom.breadcrumbs.querySelector('button[data-action="goRoot"]');
    ...
    getComputerItems() {
        ...
        const items = virtual_fs_js_1.VirtualFS.list(path);
        ...
        if (type === 'folder') {
            ...
            if (this.source === 'computer') {
                try {
                    ...
                    const content = virtual_fs_js_1.VirtualFS.readFile(pathParts);
                        ...
                        if (imageExts.has(ext)) {
                            ...
                            try {
                                ...
                                if (resp.ok) {
                                    ...
                                    if (
                                        preview_instance_manager_1.default &&
                                        typeof preview_instance_manager_1.default.openImages ===
                                            'function'
                                    ) {
                                        preview_instance_manager_1.default.openImages(
                                            [obj],
                                            0,
                                            subPath
                                        );
    ...
    navigateToFolder(name) {
        ...
        const folder = virtual_fs_js_1.VirtualFS.getFolder(targetPath);
        if (folder) {
            this.currentPath = [...this.currentPath, name];
            this._persistState();
            this._renderAll();
    ...
    _updateTabLabel() {
        const w = window;
        const t = (key, fb) => (w.appI18n ? w.appI18n.translate(key, {}, { fallback: fb }) : fb);
        let label = '';
...
FinderView.RECENT_FILES_KEY = 'finder-recent-files';
FinderView.MAX_RECENT_FILES = 20;
window.FinderView = FinderView;
//# sourceMappingURL=finder-view.js.map
