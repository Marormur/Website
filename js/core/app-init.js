...
const window_menu_1 = require('../ui/window-menu');
const session_guard_1 = require('../services/session-guard');
const instance_shims_1 = require('../compat/instance-shims');
/**
...
function initApp() {
    ...
    try {
        (0, window_menu_1.initializeWindowMenu)();
    } catch (err) {
    ...
    if (win.MultiWindowSessionManager) {
        try {
            const multiCheck = (0, session_guard_1.validateMultiWindowSession)();
            const legacyCheck = (0, session_guard_1.validateLegacySession)();
            if (legacyCheck.shouldClear) {
                console.warn('[APP-INIT] Clearing ONLY legacy session (corrupted)');
                (0, session_guard_1.clearSessionKey)('windowInstancesSession');
            }
            if (multiCheck.shouldClear) {
                console.warn('[APP-INIT] Clearing multi-window session (corrupted)');
                (0, session_guard_1.clearSessionKey)('multi-window-session');
            }
    ...
    async function markReady() {
        try {
            ...
            const sessionRestorePromise = window.__sessionRestorePromise;
            ...
            if (typeof gw2.FinderInstanceManager === 'undefined') {
                try {
                    ...
                    (0, instance_shims_1.installShim)(
                        {
                            ...
                            setActiveInstance(instanceId) {
                                try {
                                    const windows = registry.getAllWindows('finder') || [];
                                    for (const win of windows) {
                                        if (win.tabs?.has(instanceId)) {
                                            win.setActiveTab?.(instanceId);
