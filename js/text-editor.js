...
const TextEditorSystem = {
     ...
     */
    syncSaveButtonState() {
        if (!this.saveButton || !this.editor)
            return;
    ...
    insertTextAtCursor(text) {
        if (!this.editor || typeof text !== 'string')
            return;
    ...
    applyStatusState() {
        ...
        if (this.statusState.type === 'i18n') {
            const { text, translated } = this.resolveTranslation(this.statusState.key, this.statusState.params);
     ...
     */
    replaceOne() {
        if (!this.editor || !this.findInput || !this.replaceInput)
            return;
        const searchText = this.findInput.value;
