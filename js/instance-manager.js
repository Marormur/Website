...
(function () {
    ...
    class InstanceManager {
        constructor(config) {
            ...
            // If SessionManager queued pending restores for this manager type, apply them now
            try {
                const w = window;
                const sm = w.SessionManager;
                if (sm && sm._pendingRestores && sm._pendingRestores[this.type]) {
                    const pending = sm._pendingRestores[this.type];
                    // Defer slightly to ensure dependent methods are available in the manager
                    setTimeout(() => {
                        try {
                            console.log(
                                `InstanceManager(${this.type}): applying ${Array.isArray(pending) ? pending.length : 0} pending restores`
                            );
                            this.deserializeAll(pending);
                        } catch (_a) {}
                    }, 50);
                    try {
                        delete sm._pendingRestores[this.type];
                    } catch (_b) {}
                }
            } catch (_c) {}
        }
        ...
        deserializeAll(data) {
            try {
                console.log(
                    `InstanceManager(${this.type}): deserializeAll called with ${Array.isArray(data) ? data.length : 0} items`
                );
            } catch (_) {}
            ...
            try {
                console.log(
                    `InstanceManager(${this.type}): restored ${this.instances.size} instances`
                );
            } catch (_) {}
