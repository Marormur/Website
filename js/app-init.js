...
function initApp() {
    ...
    if (win.SessionManager) {
        try {
            ...
            // Use a slightly longer delay for legacy SessionManager restore to
            // give bundled instance managers time to initialize in slower CI
            // environments. If restore finds no managers it will still proceed
            // but delaying reduces race failures in Playwright tests.
            setTimeout(() => {
                try {
            ...
            }, 350); // Increased delay to reduce restore race conditions
        } catch (err) {
    ...
    function markReady() {
        try {
            ...
            // WindowRegistry - Expose for window/tab introspection (create both aliases)
            ...
            } else if (gw2.__WindowRegistry) {
                // Some modules may register under __WindowRegistry; mirror to legacy name
                gw2.WindowRegistry = gw2.__WindowRegistry;
                console.info('[APP-INIT] __WindowRegistry mirrored to WindowRegistry');
            }
            // FinderSystem - Expose for finder operations (mirror both ways)
            if (gw2.FinderSystem) {
                gw2.__FinderSystem = gw2.FinderSystem;
                console.info('[APP-INIT] FinderSystem exposed as __FinderSystem');
            } else if (gw2.__FinderSystem) {
                gw2.FinderSystem = gw2.__FinderSystem;
                console.info('[APP-INIT] __FinderSystem mirrored to FinderSystem');
            }
            // TerminalSystem - Expose for terminal operations (mirror both ways)
            ...
            } else if (gw2.__TerminalSystem) {
                gw2.TerminalSystem = gw2.__TerminalSystem;
                console.info('[APP-INIT] __TerminalSystem mirrored to TerminalSystem');
            }
            // LaunchpadSystem - Expose for launchpad operations (mirror both ways)
            ...
            } else if (gw2.__LaunchpadSystem) {
                gw2.LaunchpadSystem = gw2.__LaunchpadSystem;
                console.info('[APP-INIT] __LaunchpadSystem mirrored to LaunchpadSystem');
            }
            // SettingsSystem - Expose for settings operations (mirror both ways)
            ...
            } else if (gw2.__SettingsSystem) {
                gw2.SettingsSystem = gw2.__SettingsSystem;
                console.info('[APP-INIT] __SettingsSystem mirrored to SettingsSystem');
            }
            // SessionManager - Ensure both legacy and underscored aliases are available
            if (gw2.SessionManager) {
                gw2.__SessionManager = gw2.__SessionManager || gw2.SessionManager;
                console.info('[APP-INIT] SessionManager available (legacy)');
            } else if (gw2.__SessionManager) {
                gw2.SessionManager = gw2.__SessionManager;
                console.info('[APP-INIT] __SessionManager mirrored to SessionManager');
            }
            // DockSystem - Expose for dock operations (mirror both ways)
            if (gw2.DockSystem) {
                gw2.__DockSystem = gw2.DockSystem;
                console.info('[APP-INIT] DockSystem exposed as __DockSystem');
            } else if (gw2.__DockSystem) {
                gw2.DockSystem = gw2.__DockSystem;
                console.info('[APP-INIT] __DockSystem mirrored to DockSystem');
            }
            // ActionBus - Expose for action dispatch (mirror both ways)
            if (gw2.ActionBus) {
                gw2.__ActionBus = gw2.ActionBus;
                console.info('[APP-INIT] ActionBus exposed as __ActionBus');
            } else if (gw2.__ActionBus) {
                gw2.ActionBus = gw2.__ActionBus;
                console.info('[APP-INIT] __ActionBus mirrored to ActionBus');
            }
            // VirtualFS aliasing: support both __VirtualFS and VirtualFS
            if (gw2.__VirtualFS && typeof gw2.VirtualFS === 'undefined') {
                try {
                    gw2.VirtualFS = gw2.__VirtualFS;
                    console.info('[APP-INIT] __VirtualFS mirrored to VirtualFS');
                } catch {}
            }
            // Legacy FinderInstanceManager shim for tests that expect the old API.
            // NOTE: this is a lightweight compatibility layer that provides
            // in-memory instances and minimal APIs used by tests. It does not
            // create UI windows; it's intended to unblock legacy test assertions
            // while the app migrates to the multi-window FinderWindow model.
                ...
                try {
                    (function () {
                        ...
                        function makeInstance(opts) {
                            const id = 'finder-' + String(nextId++);
                            const inst = {
                                instanceId: id,
                                type: 'finder',
                                title: (opts && opts.title) || 'Finder',
                                currentPath: ['Computer'],
                                viewMode: 'list',
                                currentView: 'computer',
                                navigateTo(paths) {
                                    try {
                                        if (Array.isArray(paths)) this.currentPath = paths.slice();
                                        else if (typeof paths === 'string')
                                            this.currentPath = paths.split('/');
                                    } catch {}
                                },
                                setViewMode(mode) {
                                    this.viewMode = mode;
                        ...
                        // Expose a minimal FinderInstance constructor for legacy tests
                        try {
                            gw2.FinderInstance = function FinderInstance(opts) {
                                return makeInstance(opts || {});
                            };
                            console.info('[APP-INIT] FinderInstance constructor shim installed');
                        } catch (e) {
                            /* ignore */
                        }
                        console.info(
