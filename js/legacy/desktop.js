...
(function (global) {
    ...
    function clearDesktopSelection(options = {}) {
        ...
        if (blur) {
            ...
            if (prev && typeof prev.blur === 'function')
                prev.blur();
        }
    }
    function focusDesktopItemByIndex(index) {
        if (!Array.isArray(DESKTOP_ITEMS) || DESKTOP_ITEMS.length === 0)
            return;
        if (index < 0)
            index = 0;
        if (index >= DESKTOP_ITEMS.length)
            index = DESKTOP_ITEMS.length - 1;
        ...
        if (desktopArea) {
            ...
            const onPointerMove = e => {
                ...
                desktopButtons.forEach(btn => {
                    ...
                    const intersects = !(rect.right < x1 ||
                        rect.left > x2 ||
                        rect.bottom < y1 ||
                        rect.top > y2);
                    if (intersects)
